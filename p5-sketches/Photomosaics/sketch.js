...
function setup() {
  let cnv = createCanvas(size * 2, size);
  cnv.parent('canvas-holder')

  naiveFitB = createButton('Naive Fit')
  naiveFitB.parent('input-holder')
  naiveFitB.mousePressed( () => {ComputePhotomosaic(0);})

  complexFitB = createButton('Complex Fit')
  complexFitB.parent('input-holder')
  complexFitB.mousePressed( () => {ComputePhotomosaic(1);})
...
function ComputePhotomosaic(fitType) {
  for (i = 0; i < img.height; i++)
    for (j = 0; j < img.width; j++) {
      let bestFit;
      if (fitType == 0)
        bestFit = getBestFitNaive(img.get(j, i)[0], img.get(j, i)[1], img.get(j, i)[2]);
      else
        bestFit = getBestFitFancy(j, i);

      image(photos[bestFit], j * (cellW), i * (cellH));
    }

  image(imgReserve, size, 0)
}
